...
def main():
    ...
    with st.sidebar:
        ...
        # Overview metrics
        overview_stats = create_country_overview_metrics(country_data)
        st.markdown("### 📊 Quick Stats")
        
        for country_code, stats in overview_stats.items():
            country_info = COUNTRIES[country_code]
            st.markdown(f"""
            <div class="metric-container">
                <strong>{country_info['flag']} {country_info['name']}</strong><br>
                📋 Occupations: {stats['total_occupations']}<br>
                📊 Avg. 2024 Risk: {stats['avg_automation_2024']:.3f}<br>
                ⚠️ High Risk by 2050: {stats['high_risk_occupations_2050']} ({stats['high_risk_percentage']:.1f}%)
            </div>
            """, unsafe_allow_html=True)
